...
class Login(QMainWindow, Ui_MainWindow):
    def __init__(self, widget, auth, db, obj=None, *args, **kwargs):
    ...
    def login(self):
        ...
        try:
            self.auth.sign_in_with_email_and_password(email, pw)
            self.line_email.clear()
            self.line_pw.clear()
            main_window = Window(email=email, db=self.db)
    ...
    def signup(self):
        ...
        if confirm_pass != pw :
            ...
            try:
                user = self.auth.create_user_with_email_and_password(email=email, password=pw)
...
class Window(QMainWindow, Ui_MainWindow):
    def __init__(self, email, db, parent=None):
        super().__init__(parent)
        loadUi("interface.ui", self)
        self.gest_names = []
        self.known_gestures = []
        self.key_points = [0, 4, 5, 9, 13, 17, 8, 12, 16, 20]
        self.email = email
        ...
        # right menu
        self.doc_ref = self.db.collection(u'activities')
    ...
    def gesture_name(self):
        ...
        self.gest_names.append(prompt) #array gest_names isinya nama nama gestur dari var prompt
    ...
    def next_train(self): ####buat train data
        ...
        self.handData = self.find_hands.Marks(self.image)
        try:
            knownGesture = self.findDistances(self.handData[0])
        except IndexError:
            self.notify("Hands not detected.")
            return
        self.known_gestures.append(knownGesture)
        ...
        if int(self.trainCnt) == int(self.numGest):
            ...
            with open(self.trainName, 'wb') as f:
                pickle.dump(self.gest_names, f)
                pickle.dump(self.known_gestures, f)
                        
        if self.train == 0:
            self.unknownGesture = self.findDistances(self.handData[0])
            self.myGesture = self.findGesture(self.unknownGesture, self.known_gestures, self.key_points, self.gest_names, self.tol)
    ...
    def refresh_recognize(self):
        ...
        if self.file_exist == True:
            ...
            if self.train == 0:
                self.unknownGesture = self.findDistances(self.handData[0])
                self.myGesture = self.findGesture(self.unknownGesture, knownGestures, self.key_points, gestNames, self.tol)
    ...
    def displayImage(self, image, window=1):
        ...
        handData = self.find_hands.Marks(self.image)
        for hand in handData:   
            for ind in self.key_points:
                #bikin circle
...
def run_backend():
    ...
    with open(r"pyqt5-gui-pyrebase-config.json", 'r') as f:
        config = json.load(f)
    with open(r"pyqt5-gui-firebase-db-options.json", 'r') as f:
        options = json.load(f)
